...
# Email (required for customer notifications on Odoo sync failures)
...
**Product Sync**: Daily at 2 AM (syncs products from Odoo)  
**Retry Odoo Sync**: Every 5 minutes (retries failed order syncs, notifies customers)

See [Odoo Order Sync Retry System](./ODOO_ORDER_SYNC_RETRY_SYSTEM.md) for details on the retry mechanism.
