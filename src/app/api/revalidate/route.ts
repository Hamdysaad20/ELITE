...
const REVALIDATE_SECRET = process.env.REVALIDATE_SECRET || "elite-revalidate-2024";

// Valid cache tags
const VALID_TAGS = ["products", "categories", "catalog", "menu", "all"] as const;
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (process.env.NODE_ENV === "production" && providedSecret !== REVALIDATE_SECRET) {
      return jsonResponse(errorResponse("Invalid revalidation secret"), 401);
    }
    
    // Validate and normalize tags
    const tagsToRevalidate: CacheTag[] = Array.isArray(tags) 
      ? tags.filter((t: string): t is CacheTag => VALID_TAGS.includes(t as CacheTag))
      : ["catalog"];
