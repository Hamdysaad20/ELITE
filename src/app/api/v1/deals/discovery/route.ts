...
import type {
  DealDiscovery,
  DealDiscoveryPricing,
  DealDiscoveryGamification,
} from "@/types/deals";
...
function calculateCountdown(pricelistName: string): number | undefined {
  ...
  try {
    ...
    const currentMinutesAdjusted =
      currentMinutes < (window.startHour || 0) * 60 + (window.startMinute || 0)
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      console.log(
        `[DISCOVERY API ${requestId}] ✅ Loaded ${allProducts.length} products from cache`,
      );
    } catch (productsError) {
      console.error(
        `[DISCOVERY API ${requestId}] ❌ Error loading products:`,
        productsError,
      );
      return jsonResponse(
        errorResponse(
    ...
    const eligibleProducts = allProducts.filter((product) => {
      ...
      if (
        ...
        !ALLOWED_CATEGORIES.includes(product.category.name)
      )
        return false;
      return true;
    });
    ...
    for (const pricelist of pricelists) {
      try {
          ...
          if (
            ...
            if (productId && productId > 0) {
              if (computePrice === "fixed" && item.fixed_price) {
                ...
                productDealPercentages.set(
                  productId,
                  Math.abs(item.percent_price),
                );
              }
            }
          } else if (
        ...
        const categoryId = representativeProduct.categoryId
          ? parseInt(representativeProduct.categoryId, 10)
          : null;
        ...
        console.error(
          `[DISCOVERY API] Error processing pricelist ${pricelist.id}:`,
          err,
        );
        continue;
      }
    ...
    console.log(
      `[DISCOVERY API ${requestId}] ✅ Completed in ${duration}ms, returning ${discoveryDeals.length} deal(s)`,
    );
    ...
    return jsonResponse(
      errorResponse(
        `Failed to fetch deals: ${error instanceof Error ? error.message : String(error)}`,
      ),
      500,
    );
  }
