...
import { Loader2, Truck, Clock, CheckCircle2, Package, MapPin, Copy, Phone, CreditCard, ArrowLeft, Home, ShoppingBag, Coffee, Utensils } from "lucide-react";
...
export function OrderDetailCard({ orderId }: OrderDetailCardProps) {
    ...
    return (
      <div className="space-y-4 animate-fadeIn">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {[1, 2].map((i) => (
            <div key={i} className="bg-white rounded-3xl shadow-lg border-2 border-elite-burgundy/10 p-6">
  ...
  const executeReorder = async (action: "replace" | "merge") => {
    ...
    try {
      ...
      if (action === "replace") {
        clearCart();
        info(`Cart cleared. Adding ${order.items.length} ${order.items.length === 1 ? "item" : "items"}...`);
      }
      ...
      for (const item of order.items) {
        try {
          ...
          if (item.attributes && typeof item.attributes === 'object') {
            if ('formatted' in item.attributes && Array.isArray(item.attributes.formatted)) {
    ...
    <>
      ...
      <div className="flex items-center gap-2 sm:gap-3 overflow-x-auto scrollbar-hide pb-2 -mx-3 px-3 sm:mx-0 sm:px-0 snap-x snap-mandatory">
        ...
        >
          ...
          <span className="font-cabin text-sm font-bold text-elite-black">All Orders</span>
        </Link>
        <Link
          href="/profile"
          className="flex items-center gap-2 px-4 py-2.5 sm:py-3 rounded-2xl bg-white border-2 border-elite-burgundy/10 hover:border-elite-burgundy/30 hover:shadow-md transition-all whitespace-nowrap touch-manipulation active:scale-95 min-h-[44px] snap-start flex-shrink-0"
        >
          ...
          <span className="font-cabin text-sm font-bold text-elite-black">Profile</span>
        </Link>
        <Link
          href="/menu"
          className="flex items-center gap-2 px-4 py-2.5 sm:py-3 rounded-2xl bg-white border-2 border-elite-burgundy/10 hover:border-elite-burgundy/30 hover:shadow-md transition-all whitespace-nowrap touch-manipulation active:scale-95 min-h-[44px] snap-start flex-shrink-0"
        >
          ...
          <span className="font-cabin text-sm font-bold text-elite-black">Shop Menu</span>
        </Link>
      </div>
      ...
      <div className="bg-white rounded-3xl shadow-xl border-2 border-elite-burgundy/10 overflow-hidden">
        ...
        <div className={`${statusInfo.bgColor} px-4 sm:px-6 py-6 sm:py-8`}>
          ...
          <div className="flex items-center justify-between mb-4 sm:mb-0">
            ...
            <span className={`px-3 sm:px-4 py-2 rounded-2xl text-xs sm:text-sm font-cabin font-bold bg-white/90 backdrop-blur-sm ${statusInfo.color === 'text-elite-cream' ? 'text-elite-burgundy' : 'text-elite-black'} border-2 ${statusInfo.color === 'text-elite-cream' ? 'border-elite-cream/30' : 'border-elite-burgundy'} ml-auto shadow-md`}>
          ...
          <div className="flex items-start gap-3 sm:gap-4 mb-4">
            <div className={`hidden sm:flex w-12 h-12 rounded-2xl ${statusInfo.color === 'text-elite-cream' ? 'bg-elite-cream/20' : 'bg-elite-burgundy/10'} items-center justify-center flex-shrink-0 ${statusInfo.color}`}>
              {statusInfo.icon}
            </div>
            <div className="flex-1">
              <h2 className={`font-calistoga text-2xl sm:text-3xl md:text-4xl ${statusInfo.color} mb-2`}>
                {statusInfo.text}
              </h2>
              <p className={`font-cabin text-sm sm:text-base md:text-lg ${statusInfo.color} ${statusInfo.color === 'text-elite-cream' ? 'opacity-90' : 'opacity-70'}`}>
          ...
          {isInProgress && (
            <div className="mt-6">
              <div className="flex items-center justify-between mb-3">
                <span className={`font-cabin text-xs sm:text-sm font-bold uppercase tracking-wider ${statusInfo.color}`}>
                  Order Progress
                </span>
                <span className={`font-calistoga text-base sm:text-lg ${statusInfo.color}`}>
                  {getDeliveryProgress()}%
                </span>
              </div>
              <div className={`w-full ${statusInfo.progressBarBg} rounded-full h-3 sm:h-4 overflow-hidden shadow-inner`}>
                <div
        ...
        <div className="bg-gradient-to-br from-elite-burgundy via-elite-burgundy to-elite-burgundy/90 rounded-3xl shadow-xl p-5 sm:p-6 text-elite-cream relative overflow-hidden group">
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h3 className="font-calistoga text-lg sm:text-xl">Need Help?</h3>
            </div>
            <p className="font-cabin text-sm text-elite-cream/90 mb-4 leading-relaxed">
              Questions about your order? We're here to help via Facebook Messenger.
            </p>
            <button 
              onClick={handleContactSupport}
        ...
        <div className="bg-white rounded-3xl shadow-xl border-2 border-elite-burgundy/10 p-5 sm:p-6 relative overflow-hidden group hover:border-elite-burgundy/30 transition-all duration-300">
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h3 className="font-calistoga text-lg sm:text-xl text-elite-black">Loved It?</h3>
            </div>
            ...
            <button
              ...
              aria-label={isReordering ? "Adding items to cart" : "Reorder this order"}
            >
              {isReordering ? (
                <>
