 ...
 * - Fallback: Inline sync if Redis/worker unavailable
 */
    ...
    if (acquired) {
      ...
      lockCheckInterval = setInterval(async () => {
        try {
          ...
          console.error("[odoo-worker] Error renewing lock:", err);
        }
...
async function releaseDistributedLock(): Promise<void> {
  ...
  try {
    ...
    console.error("[odoo-worker] Error releasing lock:", error);
  }
}
...
async function shutdownWorker(): Promise<void> {
  ...
  try {
    console.log("[odoo-worker] Shutting down gracefully...");
    await worker.close();
    await releaseDistributedLock();
    worker = null;
    workerStarted = false;
  } catch (error) {
    console.error("[odoo-worker] Error during shutdown:", error);
  }
}
...
function attachShutdownHandlers(): void {
  ...
  const shutdown = async (signal: string) => {
    console.log(`[odoo-worker] Received ${signal}, shutting down...`);
    await shutdownWorker();
    process.exit(0);
  };
  ...
  process.on("unhandledRejection", async (reason) => {
    console.error("[odoo-worker] Unhandled rejection:", reason);
...
> | null> {
  ...
  if (isEdgeRuntime()) {
    console.log("[odoo-worker] Skipping worker start (Edge runtime detected)");
    return null;
  }
  ...
  try {
    ...
    if (isServerless()) {
      ...
      if (!lockAcquired) {
        console.log("[odoo-worker] Worker already running on another instance");
        return null;
      }
    }

    // Start the worker
    worker = startOdooWorker();
    if (worker) {
      workerStarted = true;
      ...
      worker.on("failed", (job, err) => {
        console.error(`[odoo-worker] Job ${job?.id} failed:`, err);
      });
    ...
    console.error("[odoo-worker] Failed to start:", error);
    await releaseDistributedLock();
    workerStarted = false;
  }
