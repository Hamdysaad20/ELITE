...
export async function getCircuitState(): Promise<CircuitState> {
  try {
    ...
    console.warn('[CIRCUIT-BREAKER] Failed to read state, assuming CLOSED:', err);
 ...
 */
export async function recordSuccess(config: CircuitBreakerConfig = DEFAULT_CONFIG): Promise<void> {
  try {
    const currentState = await getCircuitState();
    
    if (currentState === CircuitState.HALF_OPEN) {
      // In half-open, count successes
      const currentSuccesses = (await redisGet<number>(CIRCUIT_BREAKER_SUCCESSES_KEY)) || 0;
 ...
 */
export async function recordFailure(config: CircuitBreakerConfig = DEFAULT_CONFIG): Promise<void> {
  try {
    const currentState = await getCircuitState();
    ...
    if (currentState === CircuitState.HALF_OPEN) {
      ...
      // Count failures
      const currentFailures = (await redisGet<number>(CIRCUIT_BREAKER_FAILURES_KEY)) || 0;
      ...
      if (newFailures >= config.failureThreshold) {
        ...
        console.error(`[CIRCUIT-BREAKER] Circuit OPENED after ${newFailures} failures`);
      } else {
        await redisSet(CIRCUIT_BREAKER_FAILURES_KEY, newFailures, 3600);
        console.warn(`[CIRCUIT-BREAKER] Failure ${newFailures}/${config.failureThreshold}`);
      }
    }
 ...
 */
export async function isRequestAllowed(config: CircuitBreakerConfig = DEFAULT_CONFIG): Promise<boolean> {
  try {
    ...
    if (state === CircuitState.OPEN) {
      ...
      if (openedAtStr) {
        ...
        if (timeSinceOpen >= config.timeoutMs) {
          ...
          console.log('[CIRCUIT-BREAKER] Circuit HALF_OPEN - testing if Odoo recovered');
    ...
    console.warn('[CIRCUIT-BREAKER] Failed to check state, allowing request:', err);
    return true;
  }
}
