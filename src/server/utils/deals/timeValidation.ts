  ...
  try {
    ...
    if (window.startHour !== undefined && window.endHour !== undefined) {
      ...
      if (endMinutes < startMinutes) {
        ...
        endMinutes += 24 * 60;
        const currentMinutesAdjusted = currentMinutes < startMinutes 
          ? currentMinutes + 24 * 60 
          : currentMinutes;
        return currentMinutesAdjusted >= startMinutes && currentMinutesAdjusted < endMinutes;
      }
 ...
 */
export function isDealActive(pricelistName: string, date: Date = new Date()): boolean {
...
export function getDealTimeWindowDescription(pricelistName: string): string {
  ...
  if (window.days && window.days.length > 0) {
    ...
    } else if (window.days.length === 2 && window.days.includes(6) && window.days.includes(0)) {
      description = 'Weekends';
    } else if (window.days.length === 5 && !window.days.includes(6) && !window.days.includes(0)) {
