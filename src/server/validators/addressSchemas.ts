...
export const ADDRESS_VALIDATION = {
  ...
  SQL_INJECTION_PATTERN: /(\b(SELECT|INSERT|UPDATE|DELETE|DROP|CREATE|ALTER|EXEC|EXECUTE|UNION|SCRIPT)\b)/i,
...
const validatePhoneCountry = (phone: string, country: string = "Egypt"): boolean => {
...
const validateZipCountry = (zip: string, country: string = "Egypt"): boolean => {
  if (!zip) return true;
  
  if (country === "Egypt" || country === "EG" || country === "EGY") {
    // Egyptian postal codes are 5 digits
    return ADDRESS_VALIDATION.ZIP_EGYPT_REGEX.test(zip) || 
...
const baseAddressSchema = z.object({
  ...
  street: z
    .string()
    .min(ADDRESS_VALIDATION.STREET_MIN_LENGTH, `Street address must be at least ${ADDRESS_VALIDATION.STREET_MIN_LENGTH} characters`)
    .max(ADDRESS_VALIDATION.STREET_MAX_LENGTH, `Street address must be less than ${ADDRESS_VALIDATION.STREET_MAX_LENGTH} characters`)
    .trim()
    .transform(sanitizeInput)
    .refine(validateNoXSS, { message: "Street address contains invalid characters" })
    .refine(validateNoSQLInjection, { message: "Street address contains invalid characters" })
    .refine(validateStreetFormat, { message: "Street address must contain at least one letter" })
    .refine(
      (val) => ADDRESS_VALIDATION.STREET_REGEX.test(val),
      { message: "Street address contains invalid characters" }
    ),
  
  apartment: z
    .string()
    .min(ADDRESS_VALIDATION.APARTMENT_MIN_LENGTH, `Apartment must be at least ${ADDRESS_VALIDATION.APARTMENT_MIN_LENGTH} character if provided`)
    .max(ADDRESS_VALIDATION.APARTMENT_MAX_LENGTH, `Apartment must be less than ${ADDRESS_VALIDATION.APARTMENT_MAX_LENGTH} characters`)
    .trim()
    .transform(sanitizeInput)
    .refine((val) => !val || validateNoXSS(val), { message: "Apartment contains invalid characters" })
    .refine((val) => !val || validateNoSQLInjection(val), { message: "Apartment contains invalid characters" })
    .refine(
      (val) => !val || ADDRESS_VALIDATION.APARTMENT_REGEX.test(val),
  ...
  city: z
    .string()
    .min(ADDRESS_VALIDATION.CITY_MIN_LENGTH, `City must be at least ${ADDRESS_VALIDATION.CITY_MIN_LENGTH} characters`)
    .max(ADDRESS_VALIDATION.CITY_MAX_LENGTH, `City must be less than ${ADDRESS_VALIDATION.CITY_MAX_LENGTH} characters`)
    .trim()
    .transform(sanitizeInput)
    .refine(validateNoXSS, { message: "City name contains invalid characters" })
    .refine(validateNoSQLInjection, { message: "City name contains invalid characters" })
    .refine(
      (val) => ADDRESS_VALIDATION.CITY_REGEX.test(val),
      { message: "City name cannot contain numbers or special characters" }
    ),
  
  state: z
    .string()
    .min(ADDRESS_VALIDATION.STATE_MIN_LENGTH, `State must be at least ${ADDRESS_VALIDATION.STATE_MIN_LENGTH} characters if provided`)
    .max(ADDRESS_VALIDATION.STATE_MAX_LENGTH, `State must be less than ${ADDRESS_VALIDATION.STATE_MAX_LENGTH} characters`)
    .trim()
    .transform(sanitizeInput)
    .refine((val) => !val || validateNoXSS(val), { message: "State contains invalid characters" })
    .refine((val) => !val || validateNoSQLInjection(val), { message: "State contains invalid characters" })
    .refine(
      (val) => !val || ADDRESS_VALIDATION.STATE_REGEX.test(val),
  ...
  country: z
    ...
    .default("Egypt")
    .refine(
      (val) => ADDRESS_VALIDATION.ALLOWED_COUNTRIES.some(c => 
...
export const addressSchema = baseAddressSchema.superRefine((data, ctx) => {
  ...
  const hasAdditional = !!(data.apartment || data.state || data.zipCode || data.phone);
  
  if (!hasStreet || !hasCity || !hasAdditional) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message: "Address must include street, city, and at least one additional field (apartment, state, zip code, or phone)",
      path: ["street"],
    });
  }
});
